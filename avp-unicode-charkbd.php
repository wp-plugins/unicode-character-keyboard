...
if ( function_exists( 'is_admin' ) && is_admin() && ( ! class_exists( 'avp_unicode_charkbd' ) ) )
{
    ...
    class avp_unicode_charkbd
    {
        ...
        var $versionNumber      = 'Version 1.01';
        ...
        function avp_unicode_charkbd_admin_init()
        {
             ...
             */
            wp_register_script( 'avp-unicode-charkbd-character', plugins_url( 'js/avp-unicode-charkbd-character.js', __FILE__ ), array( 'jquery', 'jquery-validate' ) );
            wp_register_script( 'avp-unicode-charkbd-close', plugins_url( 'js/avp-unicode-charkbd-close.js', __FILE__ ), array( 'jquery' ) );
            wp_register_script( 'avp-unicode-charkbd-file', plugins_url( 'js/avp-unicode-charkbd-file.js', __FILE__ ), array( 'jquery', 'jquery-validate' ) );
            wp_register_script( 'avp-unicode-charkbd-menu', plugins_url( 'js/avp-unicode-charkbd-menu.js', __FILE__ ), array( 'jquery' ) );
            wp_register_script( 'avp-unicode-charkbd-post', plugins_url( 'js/avp-unicode-charkbd-post.js', __FILE__ ), array( 'jquery' ) );
            wp_register_script( 'avp-unicode-charkbd-postbox', plugins_url( 'js/avp-unicode-charkbd-postbox.js', __FILE__ ), array( 'jquery', 'postbox' ) );
            ...
            if ( in_array( $pagenow, array( 'admin.php', ) ) )
            {
                ...
                if ( ( 'avp-unicode-charkbd-menu-manage' == $_GET[ 'page' ] ) )
                {
                    ...
                    wp_enqueue_script( 'avp-unicode-charkbd-character' );
            ...
            {
                add_filter( ( 'plugin_action_links_' . plugin_basename( __FILE__ ) ), array( &$this, 'avp_unicode_charkbd_admin_init_action_links' ) );
                add_filter( 'plugin_row_meta', array( 'avp_unicode_charkbd', 'avp_unicode_charkbd_admin_init_meta_links' ), 10, 2 );
            }
        }
         ...
         */
        function avp_unicode_charkbd_admin_init_action_links( $links )
        {
            /*
             *  Using array_unshift puts the Settings link at the start of the sequence
             */
            array_unshift( $links, sprintf( '<a href="admin.php?page=avp-unicode-charkbd-menu" title="%s">%s</a>', __( 'Settings', 'avp-unicode-charkbd' ), __( 'Settings', 'avp-unicode-charkbd' ) ) );
         ...
         *  Add links to plugin description section
         */
        function avp_unicode_charkbd_admin_init_meta_links( $links, $file )
        {
            /*
            ...
            if ( basename( __FILE__ ) == basename( $file ) )
            {
                 ...
                 */
                array_push( $links, sprintf( '<a href="http://wordpress.org/extend/plugins/unicode-character-keyboard/">%s</a>', __( 'Download', 'avp-unicode-charkbd' ) ) );
                array_push( $links, sprintf( '<a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=THLBLFT4BV7E2">%s</a>', __( 'Donate', 'avp-unicode-charkbd' ) ) );
            }
        ...
        function avp_unicode_charkbd_admin_menu()
        {
             ...
             */
            add_submenu_page( 'avp-unicode-charkbd-menu', ( $this->title . ' Settings' ), sprintf( '<img src="%s" width="16" height="16" alt="" /> %s', plugins_url( '/images/form-input-16.png', __FILE__ ), __( 'Settings', 'avp-unicode-charkbd' ) ), 'manage_options', 'avp-unicode-charkbd-menu', array( &$this, 'avp_unicode_charkbd_admin_menu_admin' ) );
            add_submenu_page( 'avp-unicode-charkbd-menu', ( $this->title . ' Manage' ), sprintf( '<img src="%s" width="16" height="16" alt="" /> %s', plugins_url( '/images/file-xml-16.png', __FILE__ ), __( 'Manage Files', 'avp-unicode-charkbd' ) ), 'manage_options', 'avp-unicode-charkbd-menu-manage', array( &$this, 'avp_unicode_charkbd_admin_menu_manage' ) );
            add_submenu_page( 'avp-unicode-charkbd-menu', ( $this->title . ' Display' ), sprintf( '<img src="%s" width="16" height="16" alt="" /> %s', plugins_url( '/images/keyboard-16.png', __FILE__ ), __( 'Display Keyboards', 'avp-unicode-charkbd' ) ), 'manage_options', 'avp-unicode-charkbd-menu-display', array( &$this, 'avp_unicode_charkbd_admin_menu_display' ) );
            add_submenu_page( 'avp-unicode-charkbd-menu', ( $this->title . ' Customize' ), sprintf( '<img src="%s" width="16" height="16" alt="" /> %s', plugins_url( '/images/customize-16.png', __FILE__ ), __( 'Customize', 'avp-unicode-charkbd' ) ), 'manage_options', 'avp-unicode-charkbd-menu-customize', array( &$this, 'avp_unicode_charkbd_admin_menu_customize' ) );
        }
        ...
        function avp_unicode_charkbd_admin_menu_admin()
        {
            ...
            for ( $typeindex = 0, $linecount = 0; ( $typeindex < count( $this->characterSetTypes ) ); $typeindex += 1, $linecount = 0 )
            {
                ...
                <table class="form-table nav-tab-contents" <?php printf( 'id="avp_unicode_charkbd_admin_menu_panel_%u"', $typeindex ); ?> style="display:<?php echo ( ( 0 == $typeindex ) ? '' : 'none;' ) ?>" >
                    ...
                    foreach ( array_keys( $this->characterSetArrays ) as $characterSetName )
                    {
                        if ( $this->characterSetArrays[ $characterSetName ][ 'type' ] == $this->characterSetTypes[ $typeindex ] )
                        {
                            ...
                            printf( '<td><strong>%s</strong></td>', $characterSetName );

                            echo ( '<td>' );
                            printf( '<strong>%s</strong>', $this->characterSetArrays[ $characterSetName ][ 'description' ] );

                            echo ( '<br/>' );
                            _e( 'This character set contains the following subset(s):', 'avp-unicode-charkbd' );
                            echo ( ' ' );
                            ...
                            {
                                echo ( '<br />' );
                                _e( 'Character set range(s):', 'avp-unicode-charkbd' );
                                echo ( ' ' );

                                for ( $count = 0; $count < count( $this->characterSetArrays[ $characterSetName ]->characterSetRange );  ++ $count )
                                {
                                    echo ( $this->characterSetArrays[ $characterSetName ]->characterSetRange[ $count ] );
                                    echo ( $this->avp_unicode_charkbd_list_grammar( $count, count( $this->characterSetArrays[ $characterSetName ]->characterSetRange ) ) );
                                }
                            }

                            if ( 0 < count( $this->characterSetArrays[ $characterSetName ]->characterSetResource ) )
                            {
                                echo( '<br />' );
                                _e( 'Character set resource(s): ', 'avp-unicode-charkbd' );

                                for ( $count = 0; $count < count( $this->characterSetArrays[ $characterSetName ]->characterSetResource );  ++ $count )
                                {
                                    printf( '<a href="%s" title="%s" >%s</a>', $this->characterSetArrays[ $characterSetName ]->characterSetResource[ $count ], $this->characterSetArrays[ $characterSetName ]->characterSetResource[ $count ], basename( $this->characterSetArrays[ $characterSetName ]->characterSetResource[ $count ] ) );
                                    echo ( $this->avp_unicode_charkbd_list_grammar( $count, count( $this->characterSetArrays[ $characterSetName ]->characterSetResource ) ) );
                                }
                            }
                            ...
                            {
                                echo ( '<br />' );
                                _e( 'Recommended supplemental font(s) for this character set:', 'avp-unicode-charkbd' );
                                echo ( ' ' );

                                for ( $count = 0; $count < count( $this->characterSetArrays[ $characterSetName ]->characterSetFont );  ++ $count )
                                {
                                    printf( '<a href="%s">%s</a>', $this->characterSetArrays[ $characterSetName ]->characterSetFont[ $count ][ 'url' ], $this->characterSetArrays[ $characterSetName ]->characterSetFont[ $count ][ 'name' ] );
                                    echo( $this->avp_unicode_charkbd_list_grammar( $count, count( $this->characterSetArrays[ $characterSetName ]->characterSetFont ) ) );
                                }
                            }
                ...
                <table>
                    ...
                    <tr>
                        <td align="center"><input type="submit" name="submit" class="button-secondary" value="Initialize" onClick="return confirm( '<?php _e( 'Are you sure you want to initialize the selections?', 'avp-unicode-charkbd' ); ?>' );" /></td>
            ...
            function avp_unicode_charkbd_admin_menu_admin_help( $help )
            {
                $help .= '<hr />';
                $help .= '<p>';
                $help .= sprintf( '<strong>%s</strong> - %s.', $this->title, $this->versionNumber );
                $help .= '<br />';
            ...
            {
                $tabNames = array( __( 'Uploading File', 'avp-unicode-charkbd' ), __( 'Deleting File', 'avp-unicode-charkbd' ), __( 'Error Reporting', 'avp-unicode-charkbd' ), __( 'Find Character', 'avp-unicode-charkbd' ) );
                ...
                if ( ( isset( $_REQUEST[ 'action' ] ) ) && ( 'delete' == $_REQUEST[ 'action' ] ) )
                {
                    ...
                    else
                    {
                        ...
                        for ( $index = 0; ( $index < count( $_POST[ 'deletebox' ] ) ); $index += 1 )
                        {
                            $characterSetName = $_POST[ 'deletebox' ][ $index ];
                             ...
                             *  Accumulate Report
                             */
                            $fileList .= $characterSetName;
                            if ( ( count( $_POST[ 'deletebox' ] ) - 2 ) > $index )
                            {
                                $fileList .= ', ';
                            }
                            else
                            if ( ( count( $_POST[ 'deletebox' ] ) - 1 ) > $index )
                            {
                                $fileList .= sprintf( ' %s ', __( 'and', 'avp-unicode-charkbd' ) );
                            }
                            else
                ...
                /*
                 *  Display list of characters found
                 */
                else
                if ( ( isset( $_REQUEST[ 'action' ] ) ) && ( 'find' == $_REQUEST[ 'action' ] ) && ( isset( $_POST[ 'character' ] ) ) )
                {
                    $tabNumber = 3;

                    if ( ( ! isset( $_REQUEST[ '_wpnonce' ] ) ) || ( ! wp_verify_nonce( $_REQUEST[ '_wpnonce' ], 'manage-find' ) ) )
                    {
                        wp_die( __( 'Security check', 'avp-unicode-charkbd' ) );
                    }
                }
                ...
                <div id="avp_unicode_charkbd_admin_menu_panel_1" style="display: <?php echo ( ( 1 == $tabNumber ) ? '' : 'none;' ) ?>" class="nav-tab-contents" >
                    ...
                    <form method="post" enctype="multipart/form-data" >
                        ...
                        <table class="form-table" >
                            ...
                            <tfoot>
                                <tr>
                                    <td align="center"><input type="submit" name="delete" class="button-primary" value="Delete" onClick="return confirm( '<?php _e( 'Are you sure you want to delete the selected file(s)?', 'avp-unicode-charkbd' ); ?>' );" /></td>
                ...
                <?php
                echo( '<!-- Error reporting section -->' );
                echo( '<div id="avp_unicode_charkbd_admin_menu_panel_2" style="display: ' . ( ( 2 == $tabNumber ) ? '' : 'none;' ) . '" class="nav-tab-contents" >' );
                echo( '<h2>' );
                _e( 'Error Reporting', 'avp-unicode-charkbd' );
                echo( '</h2>' );
                ...
                {
                    printf( '%s', __( 'Error log file empty, unable to display.', 'avp-unicode-charkbd' ) );
                }
                else
                {
                    ...
                    echo( '<h3 class="hndle"><span>error.log</span></h3>' );
                    echo( '<div class="inside">' );
                    echo( '<pre>' );
                    echo( $this->avp_unicode_charkbd_return_file_contents( plugin_dir_path( __FILE__ ) . '/error.log' ) );
                    echo( '</pre>' );
                    echo( '</div>' );
                    echo( '</div>' );
                    echo( '</div>' );
                    ...
                    echo wp_nonce_field( 'manage-error' );
                    echo( '<input type="hidden" name="action" value="error" />' );
                    echo( '<label for="delete">' );
                    echo( '<input type="submit" name="delete" class="button-primary" value="Delete" onClick="return confirm( \'' . __( 'Are you sure you want to delete the log file?', 'avp-unicode-charkbd' ) . '\' );" />' );
                    _e( '<strong>To delete the error log file, click the "Delete" button.</strong>', 'avp-unicode-charkbd' );
                    echo( '</label>' );
                    echo( '</form>' );
                }
                ...
                echo( '<!-- Character search reporting section -->' );
                echo( '<div id="avp_unicode_charkbd_admin_menu_panel_3" style="display: ' . ( ( 3 == $tabNumber ) ? '' : 'none;' ) . '" class="nav-tab-contents" >' );
                echo( '<h2>' );
                _e( 'Find Character', 'avp-unicode-charkbd' );
                echo( '</h2>' );
                echo( '<p>To find a specific character within the existing character sets, enter the Unicode (literal, decimal or hexadecimal) value of the character or the actual character in the input box and press "Return" or click the "Find" button. As with Unicode values, the character definition is case-sensitive.</p>' );

                if ( ( isset( $_REQUEST[ 'action' ] ) ) && ( 'find' == $_REQUEST[ 'action' ] ) && ( isset( $_POST[ 'character' ] ) ) )
                {
                    /*
                     *  Encode the character entry 
                     */
                    if ( 1 == mb_strlen( $_POST[ 'character' ] ) )
                    {
                        $character = $_POST[ 'character' ];
                        $testChar  = true;
                    }
                    else
                    if ( preg_match( '/^&(([a-z0-9]+)|(#[0-9]+)|(#x[a-f0-9]+));$/iu', $_POST[ 'character' ] ) )
                    {
                        $character = html_entity_decode( $_POST[ 'character' ], ENT_QUOTES, 'UTF-8' );
                        $testChar  = true;
                    }
                    else
                    if ( preg_match( '/^(([0-9]+)|(x[a-f0-9]+))$/iu', $_POST[ 'character' ] ) )
                    {
                        $character = html_entity_decode( ( '&#' . $_POST[ 'character' ] . ';' ), ENT_QUOTES, 'UTF-8' );
                        $testChar  = true;
                    }
                    else
                    if ( preg_match( '/^([a-z0-9]+)$/iu', $_POST[ 'character' ] ) )
                    {
                        $character = html_entity_decode( ( '&' . $_POST[ 'character' ] . ';' ), ENT_QUOTES, 'UTF-8' );
                        $testChar  = true;
                    }
                    ...
                    {
                        echo( '<h3>' );
                        printf( __( 'Search for: invalid character input "%s" specified.', 'avp-unicode-charkbd' ), $_POST[ 'character' ] );
                        echo( '</h3>' );
                    }
                    else
                    {
                        $foundFlag = false;
                        echo( '<h3>' );
                        printf( __( 'Search for: character "%s"', 'avp-unicode-charkbd' ), htmlspecialchars( $character ) );
                        echo( '</h3>' );
                        ...
                        foreach ( array_keys( $this->characterSetArrays ) as $characterSetName )
                        {
                            foreach ( $this->characterSetArrays[ $characterSetName ]->characterSubset as $subset )
                            {
                                foreach ( $subset as $array )
                                {
                                    ...
                                    {
                                        if ( html_entity_decode( ( ( string ) $key[ 'character' ] ), ENT_QUOTES, 'UTF-8' ) === $character )
                                        {
                                            $foundFlag = true;
                                            printf( __( 'Character %s found: character set %s, subset %s, Unicode specifier "%s" with description "%s".', 'avp-unicode-charkbd' ), ( ( string ) $key[ 'character' ] ), $characterSetName, $subset[ 'name' ], $this->avp_unicode_charkbd_format_char( ( string ) $key[ 'character' ] ), $this->avp_unicode_charkbd_format_text( $key ) );
                                            echo( '<br/>' );
                                        }
                                    }
                                }
                        ...
                        if ( ! $foundFlag )
                        {
                            printf( __( '<strong>Character %s not found in any defined character set.</strong>', 'avp-unicode-charkbd' ), htmlspecialchars( $character ) );
                        }
                ...
                echo( '<p>' );
                echo( '<form method="post" enctype="multipart/form-data" id="findCharacter" >' );
                echo wp_nonce_field( 'manage-find' );
                echo( '<input type="hidden" name="action" value="find" />' );
                echo( '<label for="find">' );
                echo( '<input type="submit" name="find" class="button-primary" value="Find" />' );
                echo( '<input type="text" maxlength="8" size="8" name="character" id="character" >' );
                echo( '<br/>' );
                echo( '</label>' );
                echo( '</form>' );
                echo( '</p>' );
                        ...
                        if ( $this->characterSetTypes[ $typeindex ] == ( string ) $this->characterSetArrays[ $characterSetName ][ 'type' ] )
                        {
                            ...
                            foreach ( $this->characterSetArrays[ $characterSetName ]->characterSubset as $subset )
                            {
                                ...
                                foreach ( $subset as $array )
                                {
                                    foreach ( $array as $key )
                                    {
                                        printf( '<abbr title="%s &mdash; %s" >%s</abbr>', $this->avp_unicode_charkbd_format_char( $key[ 'character' ] ), $this->avp_unicode_charkbd_format_text( $key ), $key[ 'character' ] );
                                    }
            ...
            function avp_unicode_charkbd_admin_menu_customize()
            {
                ...
                <div id="avp_unicode_charkbd_admin_menu_panel_2" style="display: none;" class="nav-tab-contents" >
                    ...
                    <div style="clear: both;"></div>
                    <p>The numeric range and optional description of the character set or subset within the ISO definition. Any number of ranges may be defined in the file. Only upper case and lower case letters, numbers, parentheses and brackets, underscores, dashes and spaces are permitted in range descriptions.</p>
                ...
                <div id="avp_unicode_charkbd_admin_menu_panel_3" style="display: none;" class="nav-tab-contents" >
                    ...
                    <div style="clear: both;"></div>
                    <p>Each character is defined by this element. Character elements can be stated in the numeric HTML character equivilance, either decimal or hexadecimal. HTML literals can be substituted for the numeric character codes but the XML parser requires the use of the "<?php echo htmlentities( '&amp;' ) ?>" to escape the preceeding "<?php echo htmlentities( '&' ) ?>".</p>
                    <p>Note: invalid character definitions cause the XML parser to return an invalid result, resulting in the file parser rejecting the entire XML file. Invalid character definitions include incorrect formatting of the character value, improper hexadecimal values, etc.</p>
                </div>
            ...
            function avp_unicode_charkbd_admin_menu_message( $type, $message, $information = '&nbsp;' )
            {
                ...
                printf( '<a style="float:right; cursor:pointer;" >%s</a>', __( 'Close', 'avp-unicode-charkbd' ) );
            ...
            function avp_unicode_charkbd_admin_menu_tablist( $tabNames, $tabNumber = 0 )
            {
                ...
                {
                    printf( '<a title="%s" rel="avp_unicode_charkbd_admin_menu_panel_%u" style="cursor:pointer;" class="nav-tab %s" >%s</a>', sprintf( '%s %s', __( 'Click to change display to', 'avp-unicode-charkbd' ), $tabNames[ $tabindex ] ), $tabindex, ( ( $tabNumber == $tabindex ) ? 'nav-tab-active' : '' ), $tabNames[ $tabindex ] );
                }
            ...
            function avp_unicode_charkbd_add_meta_box()
            {
                ...
                foreach ( array_keys( $this->characterSetArrays ) as $characterSetName )
                {
                    if ( get_option( $this->avp_unicode_charkbd_setting_name( $characterSetName ) ) )
                    {
                        printf( '<a title="%s" rel="avp_unicode_charkbd_%s" class="avp-meta-box-control" style="cursor:pointer;" >', sprintf( __( 'Click to toggle the display of %s special characters', 'avp-unicode-charkbd' ), $characterSetName ), $this->avp_unicode_charkbd_setting_name( $characterSetName ) );
                        if ( 'Common' == $characterSetName )
                            echo ( '<strong>' );
                    ...
                    if ( get_option( $this->avp_unicode_charkbd_setting_name( $characterSetName ) ) )
                    {
                        ...
                        foreach ( $this->characterSetArrays[ $characterSetName ]->characterSubset as $subset )
                        {
                            ...
                            foreach ( $subset as $array )
                            {
                                foreach ( $array as $key )
                                {
                                    printf( '<abbr title="%s &mdash; %s" onClick="javascript:send_to_editor( \'%s\' );" >%s</abbr>', $this->avp_unicode_charkbd_format_char( $key[ 'character' ] ), $this->avp_unicode_charkbd_format_text( $key ), $key[ 'character' ], $key[ 'character' ] );
                                }
            ...
            function avp_unicode_charkbd_getdir( $directory )
            {
                if ( ( $handle = @opendir( $directory ) ) )
                {
                    while ( false !== ( $filename = @readdir( $handle ) ) )
                    {
                        ...
                        if ( ! preg_match( '/\.xml$/iu', $filename ) )
                        {
                            $this->avp_unicode_charkbd_error_log( $filename, 'Invalid file in module directory' );
                        }
            ...
            function avp_unicode_charkbd_parse_xml( $filename, $addArray = true, $printFile = false )
            {
                ...
                {
                    $this->avp_unicode_charkbd_error_log( $filename, 'Error: "Help" is a prohibited character set name.' );
                    return( false );
                }
                /*
                ...
                if ( isset( $characterSet->characterSetFont ) )
                {
                    ...
                    {
                        if ( ( '' == ( string ) $characterSet->characterSetFont[ $count ][ 'name' ] ) )
                        {
                            $this->avp_unicode_charkbd_error_log( $filename, sprintf( 'Error: missing font name in <characterSetFont> element # %u.', ( $count + 1 ) ) );
                            return( False );
                        }
                        else
                        if ( ( '' == ( string ) $characterSet->characterSetFont[ $count ][ 'url' ] ) )
                        {
                            $this->avp_unicode_charkbd_error_log( $filename, sprintf( 'Error: missing font URL parameters in <characterSetFont> element # %u.', ( $count + 1 ) ) );
                            return( False );
                        }
                        /*
                         *  Validate parameters
                        ...
                        {
                            $this->avp_unicode_charkbd_error_log( $filename, sprintf( 'Error: invalid font name in <characterSetFont> element # %u, parameter contents [%s]. Font names nust consist only of letters, numbers and spaces.', ( $count + 1 ), ( string ) $characterSet->characterSetFont[ $count ][ 'name' ] ) );
                            return( False );
                        }
                        else
                        if ( ! filter_var( ( string ) $characterSet->characterSetFont[ $count ][ 'url' ], FILTER_VALIDATE_URL, FILTER_FLAG_PATH_REQUIRED ) )
                        {
                            $this->avp_unicode_charkbd_error_log( $filename, sprintf( 'Error: invalid URL in <characterSetFont> element # %u, parameter contents [%s].', ( $count + 1 ), ( string ) $characterSet->characterSetFont[ $count ][ 'url' ] ) );
                            return( False );
                        }
                    }
                }
                ...
                if ( isset( $characterSet->characterSetRange ) )
                {
                    ...
                    {
                        if ( ( '' == ( string ) $characterSet->characterSetRange[ $count ] ) )
                        {
                            $this->avp_unicode_charkbd_error_log( $filename, sprintf( 'Error: missing font range parameter value in <characterSetRange> element # %u.', ( $count + 1 ) ) );
                            return( false );
                        }
                        else
                        if ( ! ctype_print( ( string ) $characterSet->characterSetRange[ $count ] ) )
                        {
                            $this->avp_unicode_charkbd_error_log( $filename, sprintf( 'Error: invalid font range parameter value in <characterSetRange> element # %u, parameter contents [%s]).', ( $count + 1 ), $characterSet->characterSetRange[ $count ] ) );
                            return( false );
                        }
                    }
                }
                ...
                {
                    for ( $count = 0; $count < count( $characterSet->characterSetResource );  ++ $count )
                    {
                        if ( '' == ( string ) $characterSet->characterSetResource[ $count ] )
                        {
                            $this->avp_unicode_charkbd_error_log( $filename, sprintf( 'Warning: missing set resource URL parameter in <characterSetResource> element # %u.', ( $count + 1 ) ) );
                            return( false );
                        }
                        //  URL value must be a properly defined HTML url
                        else
                        if ( ! filter_var( ( string ) $characterSet->characterSetResource[ $count ], FILTER_VALIDATE_URL, FILTER_FLAG_PATH_REQUIRED ) )
                        {
                            $this->avp_unicode_charkbd_error_log( $filename, sprintf( 'Warning: invalid set resource URL parameter in <characterSetResource> element # %u, parameter contents [%s]).', ( $count + 1 ), $characterSet->characterSetResource[ $count ] ) );
                            return( false );
                        }
                    }
                }
                ...
                {
                    $this->avp_unicode_charkbd_error_log( $filename, 'Error: missing or empty <characterSubset> elements.' );
                    return( false );
                }

                for ( $count = 0; $count < count( $characterSet->characterSubset );  ++ $count )
                {
                    //  Test subset name
                    if ( ( '' == ( string ) $characterSet->characterSubset[ $count ][ 'name' ] ) || ( ! ctype_print( ( string ) $characterSet->characterSubset[ $count ][ 'name' ] ) ) )
                    {
                        $this->avp_unicode_charkbd_error_log( $filename, 'Error: missing or invalid name parameter in <characterSubset> element.' );
                        return( false );
                    }
                    else
                    if ( 0 == count( $characterSet->characterSubset[ $count ] ) )
                    {
                        $this->avp_unicode_charkbd_error_log( $filename, 'Error: empty <characterSubsetArray> element.' );
                        return( false );
                    }

                    foreach ( $characterSet->characterSubset[ $count ] as $array )
                    {
                        foreach ( $array as $key )
                        {
                            if ( ( '' === ( string ) $key[ 'character' ] ) || ( ! preg_match( '/^&(([a-z0-9]+)|(#[0-9]+)|(#x[a-f0-9]+));$/iu', ( string ) $key[ 'character' ] ) ) )
                            {
                                $this->avp_unicode_charkbd_error_log( $filename, sprintf( 'Error: missing or invalid character name parameters in <characterDefinition> element, character [%s].', ( string ) $key[ 'character' ] ) );
                                return( false );
                            }
                            else
                            if ( ( '' === ( string ) $key ) || ( ! ctype_print( ( string ) $key ) ) )
                            {
                                $this->avp_unicode_charkbd_error_log( $filename, sprintf( 'Error: missing or invalid character definition parameters in <characterDefinition> element, definition [%s].', ( string ) $key ) );
                                return( false );
                            }
                        }
                    }
            ...
            /*
             *  Reformat character to use hexadecimal numerals
             */
            function avp_unicode_charkbd_format_char( $character )
            {
                $array = array( );

                /*
                 *  Translate decimal values to hexadecimal values
                 *  Zero fill the hex value out to four characters to match the Unicode PDFs, any more than four characters are left alone
                 *  Negative values to the last parameter for the substr call cause the routine to return characters from the end of the string
                 */
                if ( preg_match( '/^&#([0-9]+);$/iu', $character, $array ) )
                {
                    $hexchar   = strtoupper( dechex( $array[ 1 ] ) );
                    $character = '&#x' . substr( "0000", 0, max( 0, ( 4 - strlen( $hexchar ) ) ) ) . $hexchar . ';';
                }
            ...
            {
                return( str_replace(
                                array( 'Apl', 'And ', 'For ', 'In ', 'On ', 'To ', 'With ', 'Without ' ), array( 'APL', 'and ', 'for ', 'in ', 'on ', 'to ', 'with ', 'without ' ), ucwords( strtolower( $text ) ) ) );
            }
